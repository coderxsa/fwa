...
class TemplateButtonsBuilder {
    ...
    addURL(opts) {
        if (!opts.displayText || !opts.url)
            throw new Error('[n3bu1a] template button builder need url display text or url');
    ...
    addCall(opts) {
        if (!opts.displayText || !opts.phoneNumber)
            throw new Error("[n3bu1a] template button builder need call display text or phone number");
        let index = this.index + 1;
    ...
    addQuickReply(opts) {
        if (!opts.displayText || !opts.id)
            throw new Error("[n3bu1a] template button builder need quick reply display text or id");
        let index = this.index + 1;
